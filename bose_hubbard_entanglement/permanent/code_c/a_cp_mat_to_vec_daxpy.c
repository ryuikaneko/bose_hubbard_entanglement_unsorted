...
// lapack
void daxpy_(uint64_t* n, double* alpha, double* x, uint64_t *incx, double* y, uint64_t *incy);
...
//    direction = 2 * cmp(old_gray,new_gray);
    direction = (double) (2 * cmp(old_gray,new_gray));
//    for(i=0; i<n; i++){
//      row_comb[i] += M[gray_diff_index][i] * direction;
//    }
    M1 = M[gray_diff_index];
//    for(i=0; i<n; i++){
//      row_comb[i] += M1[i] * direction;
//    }
    daxpy_(&n,&direction,M1,&incx,row_comb,&incy);
    sign = -sign;
    old_gray = new_gray;
  }
  free(row_comb);
