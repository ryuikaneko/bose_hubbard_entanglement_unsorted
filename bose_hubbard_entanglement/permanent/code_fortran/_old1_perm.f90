...
contains
  function cmp_dirty(a,b) result(ret) !! works only for gfortran
    integer(ip) :: a,b,ret
    ret = transfer(a>b,1_ip) - transfer(a<b,1_ip)
  end function
!
...
!      write(*,*) new_gray
!      write(*,*) gray_diff
!      write(*,*) gray_diff_index
!      write(*,*) cmp_dirty(old_gray,new_gray)
!      write(*,*) cmp(old_gray,new_gray)
!
!      direction = 2_ip * cmp_dirty(old_gray,new_gray)
      direction = 2_ip * cmp(old_gray,new_gray)
      do i=1, n
        row_comb(i) = row_comb(i) + M(gray_diff_index+1,i) * direction
!        write(*,*) direction, M(gray_diff_index+1,i)
      end do
      sign = -sign
      old_gray = new_gray
    end do
