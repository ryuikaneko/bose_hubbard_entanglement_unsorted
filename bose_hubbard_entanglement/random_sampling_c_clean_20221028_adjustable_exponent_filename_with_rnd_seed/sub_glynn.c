...
int glynn_var(double complex **matA, int L,
  ...
  for(i=0; i<Nblock; i++){
    ...
    for(j=0; j<Nblocksize; j++){
      tmp_perm = glynn_child(matA,L,vec,tmp);
      tmp_ave += tmp_perm;
      tmp_var_r +=  creal(tmp_perm)*creal(tmp_perm);
      tmp_var_i +=  cimag(tmp_perm)*cimag(tmp_perm);
    }
    tmp_var_r = tmp_var_r/Nblocksize - creal(tmp_ave)/Nblocksize * creal(tmp_ave)/Nblocksize;
    tmp_var_i = tmp_var_i/Nblocksize - cimag(tmp_ave)/Nblocksize * cimag(tmp_ave)/Nblocksize;
    histblock[i] = tmp_ave/Nblocksize;
    histvarblock[i] = tmp_var_r + I*tmp_var_i;
  }
  return 0;
}
...
int blocking_bootstrap(uint64_t Nblock, uint64_t Nboot, double complex *histblock,
  ...
  for(i=0; i<Nboot; i++){
    tmp = 0.0;
    for(j=0; j<Nblock; j++){
      tmp += histblock[ pcg32_boundedrand(Nblock) ];
    }
    tmp_block[i] = tmp/Nblock;
  }
  // average
  ave_re = 0.0;
  ave_im = 0.0;
  ...
  err_re = sqrt(err_re/Nboot);
  err_im = sqrt(err_im/Nboot);
  val[0] = ave_re + I*ave_im;
  val[1] = err_re + I*err_im;
  return 0;
}
