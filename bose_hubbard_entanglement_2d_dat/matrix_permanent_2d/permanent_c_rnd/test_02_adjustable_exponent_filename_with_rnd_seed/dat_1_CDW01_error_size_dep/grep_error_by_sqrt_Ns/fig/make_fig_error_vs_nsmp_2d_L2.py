...
#    skips = np.array([0,0,0,0,0,10,10,12])
    Ls = np.array([[4,4],[6,6],[8,8],[10,10]])
    skips = np.array([0,0,0,10])
    Ls = Ls[::-1]
    skips = skips[::-1]
    dat = []
    for L in Ls:
...
#----
    ...
    aas = np.array(aas)
    #print(aas)
    np.savetxt("dat_coeff_2d_L2",aas)
    ...
    fig.savefig("fig_error_vs_nsmp_2d_L2.pdf",bbox_inches="tight")
    plt.close()

#----

    mask = (aas[:,0]*aas[:,1]>=16) ## fit for Lx*Ly>=16
    #mask = (aas[:,0]*aas[:,1]>=36) ## fit for Lx*Ly>=36
    print(mask)
    print(aas[mask,0])
    slope, slopeerr, shift, shifterr = fit_exp(aas[mask,0]*aas[mask,1],aas[mask,2],aas[mask,3])
    np.savetxt("dat_slope_2d_L2",np.array([slope,slopeerr,shift,shifterr]).reshape(1,4))
