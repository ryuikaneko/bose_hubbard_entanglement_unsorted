...
// prameter settings
  ...
  L_A = L/2;
  Nsmp = (uint64_t) pow(2.0,L);
//  Nsmp = (uint64_t) pow(2.0,L*0.60);// (ln3)/2 = 0.54930614433
//  Nsmp = (uint64_t) pow(2.0,L*0.35);
//  Nsmp = 1LLU<<32;
//  Nsmp = 1<<30;
//  Nsmp = 1<<((int)(L/4)+4);
//  printf("%d\n",Nsmp);
...
//    glynn(matA,L,vec,tmp,Nsmp,val);
    glynn(matA,2*L,vec,tmp,Nsmp,rng,val);// MI
//    glynn(matA,L,vec,tmp,Nsmp,rng,val);// CDW
