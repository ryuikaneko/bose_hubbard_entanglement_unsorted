...
def parse_args():
    parser = argparse.ArgumentParser(description="Bose-Hubbard chain, entanglement")
    parser.add_argument("-L",metavar="L",dest="L",type=int,default=16,help="set L(=16, size)")
...
def get_dat_2(L,system,J,ts,Nsmp,period=1):
    rs, ks = calc_rsks(L)
    eks = calc_eks(J,ks)
    xkrs = calc_xkrs(L,ks,rs)
    data = np.zeros((len(ts),2),dtype=np.float64)
    ...
    for i,t in enumerate(ts):
        ...
        for step in range(Nsmp):
            ...
            rnd = np.pi * np.random.uniform(-1.0,1.0,L)
...
def calc_renyi(period,J,ts,Ls,Nsmp):
    ...
    for i,L in enumerate(Ls):
        ...
        datas[i], datas2[i], datas2err1[i], datas2err2[i] = get_dat_2(L,system,J,ts,Nsmp,period)
        #end = time.time()
...
#    Nsmp = 2**(Ls[0]//4+4)
    ...
    datas, datas2, datas2err1, datas2err2 = calc_renyi(period,J,ts,Ls,Nsmp)
    end = time.time()
    print("L,calctime:",Ls[0],end-start)
    np.savetxt("dat_L"+"{}".format(Ls[0])+"_t"+"{:.10f}".format(ts[0])+"_calctime",[end-start])
    np.savetxt("dat_L"+"{}".format(Ls[0])+"_t"+"{:.10f}".format(ts[0])+"_ave",datas2[0])
    np.savetxt("dat_L"+"{}".format(Ls[0])+"_t"+"{:.10f}".format(ts[0])+"_err1",datas2err1[0])
    np.savetxt("dat_L"+"{}".format(Ls[0])+"_t"+"{:.10f}".format(ts[0])+"_err2",datas2err2[0])
